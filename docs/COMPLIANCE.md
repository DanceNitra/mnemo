...
| Article | Title | inspeximus primitive(s) | How it supports the obligation |
|---|---|---|---|
| **Art. 12** | Record-keeping (automatic logging over lifetime) | Hash-linked write receipts; `history()`; `as_of()`; supersession ledger | Automatic, tamper-evident, timestamped logging of memory events supports traceability. |
| **Art. 19** | Automatically generated logs (retention) | Append-only receipt chain; `anchor()` | Append-only receipts with a signed tree head support retention and integrity of automatically generated logs. |
| **Art. 10** | Data and data governance | Attestation; `check_conflict`; `detect_pii` / `redact_pii`; per-type decay | Provenance, contradiction gating, and PII handling contribute to data-governance evidence. |
| **Art. 15** | Accuracy, robustness and cybersecurity | Supersession; poison-defense / influence gate; hash-linked receipts; witness co-signing | Integrity, correction, and anti-tampering primitives contribute to accuracy and cybersecurity properties. |
