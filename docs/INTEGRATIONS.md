...
|---|---|---|
| [LangChain](#current-truth-retriever-for-langchain-mnemoretriever-1110) | `MnemoRetriever` | `pip install "inspeximus[langchain]"` |
| [OpenAI Agents SDK](#drop-in-memory-for-the-openai-agents-sdk-mnemosession-0620) | `MnemoSession` | `pip install "inspeximus[openai-agents]"` |
| [AutoGen](#current-truth-memory-for-autogen-mnemomemory-070) | `MnemoMemory` | `pip install "inspeximus[autogen]"` |
| [LangGraph / LangMem](#langgraph-store-with-queryable-history-mnemostore-071) | `MnemoStore` | `pip install "inspeximus[langgraph]"` · [runnable example](examples/07_langgraph_memory.py) |
| [LlamaIndex](#current-truth-long-term-memory-for-llamaindex-mnemomemoryblock-073) | `MnemoMemoryBlock` | `pip install "inspeximus[llamaindex]"` |
| [Google ADK](#persistent-memory-for-google-adk-mnemomemoryservice-074) | `MnemoMemoryService` | `pip install "inspeximus[google-adk]"` |
| [Pydantic AI](#memory-as-tools-for-pydantic-ai-mnemo_toolset-078) | `mnemo_toolset` | `pip install "inspeximus[pydantic-ai]"` |
| [CrewAI](#current-truth-storage-for-crewai-mnemostorage-1120) | `MnemoStorage` | `pip install "inspeximus[crewai]"` |

Details for each below.

### Current-truth retriever for LangChain: `MnemoRetriever` (1.11.0+)
`inspeximus.integrations.langchain.MnemoRetriever` is a LangChain [`BaseRetriever`](https://python.langchain.com/docs/concepts/retrievers/)
...
`inspeximus.integrations.openai_agents.MnemoSession` is a persistent [`Session`](https://openai.github.io/openai-agents-python/sessions/)
...
`inspeximus/probes/mnemo_session_adapter_probe.py` (11/11). Adapters live under `inspeximus.integrations` (opt-in extras).

### Current-truth memory for AutoGen: `MnemoMemory` (0.7.0+)
`inspeximus.integrations.autogen.MnemoMemory` implements AutoGen's [`Memory`](https://microsoft.github.io/autogen/stable/user-guide/agentchat-user-guide/memory.html)
...
Verified end-to-end against the real `autogen-core` (`inspeximus/probes/mnemo_autogen_adapter_probe.py`, 7/7,
...
`inspeximus.integrations.langgraph.MnemoStore` is a LangGraph [`BaseStore`](https://langchain-ai.github.io/langgraph/reference/store/)
...
value is gone; `MnemoStore` keeps it on inspeximus's supersession ledger, so `store.history(namespace, key)` returns
...
Verified end-to-end against real `langgraph` (`inspeximus/probes/mnemo_langgraph_adapter_probe.py`, 9/9, incl. the
...
`inspeximus.integrations.llamaindex.MnemoMemoryBlock` is a LlamaIndex long-term [`BaseMemoryBlock`](https://developers.llamaindex.ai/python/framework/module_guides/deploying/agents/memory/)
...
memory = Memory.from_defaults(session_id="s1", token_limit=40000,
                              memory_blocks=[MnemoMemoryBlock(name="inspeximus", path="mem.json", k=5)])
```
...
(`inspeximus/probes/mnemo_llamaindex_adapter_probe.py`, 4/4, incl. "corrected value not re-injected"). Subclasses
...
`inspeximus.integrations.google_adk.MnemoMemoryService` is a drop-in Google ADK [`BaseMemoryService`](https://google.github.io/adk-docs/sessions/memory/)
...
2.4.0 (`inspeximus/probes/mnemo_adk_adapter_probe.py`, 4/4, incl. per-user isolation, current-truth, and
...
etc.) is to expose memory as agent tools. `inspeximus.integrations.pydantic_ai.mnemo_toolset` returns a
...
(`inspeximus/probes/mnemo_pydantic_ai_adapter_probe.py`). Importing this module imports Pydantic AI (opt-in
...
`save(value, metadata)` / `search(query, limit, score_threshold)` / `reset()`. `inspeximus.integrations.crewai.MnemoStorage`
...
`inspeximus/probes/mnemo_crewai_adapter_probe.py` (6/6, incl. "corrected value not returned").
...
```python
from inspeximus import Mnemo, new_receipt_keypair, sign_revert
