...
(what most RAG does) vs mnemo, which tracks currency (keyed supersession + revert) and provenance (a warrant signal)
...
agora_output/lab/integrity_conditioned_recall.py (revert: mnemo 1.0 vs recency 0.0; poison: only mnemo+warrant 1.0).
"""
...
def naive_rag(texts, query):
    ...
    scratch = Mnemo(path=None)                      # lexical scoring, zero-dep
    for t in texts:
        scratch.remember(t)
...
def show(title, story, correct, naive_answer, mnemo_answer):
    print("\n" + "=" * 78 + f"\n{title}\n{story}")
    print(f"  CORRECT answer          : {correct}")
    ok = lambda a: "OK " if a == correct else "XX "
    print(f"  [{ok(naive_answer)}] plain store (RAG)        : {naive_answer}")
    print(f"  [{ok(mnemo_answer)}] mnemo                    : {mnemo_answer}")


# ── 1. SUPERSESSION — a fact changed. RAG still has both; mnemo returns the current one. ──────────────
...
# ── 2. REVERT — a wrong update was rolled back. RAG is stuck on the bad value; mnemo restores the prior. ──
correct = "My medication dose is 20 mg."
...
# ── 3. POISON — an injected memory is engineered to look most relevant. mnemo demotes the unwarranted one. ──
...
print("\n" + "=" * 78)
print("Plain nearest-match memory returns the stale / rolled-back / poisoned fact. mnemo returns the")
print("current, trustworthy one — deterministically, no LLM on the read path. Full n=100 benchmark + CIs:")
print("https://github.com/DanceNitra/mnemo  (see the integrity-conditioned recall benchmark).")
